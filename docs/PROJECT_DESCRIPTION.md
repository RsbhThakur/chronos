...
To help hackathon judges easily evaluate Chronos' deep features without requiring tedious account setup, API integrations, or real task completion cycles, we have built a **fully functional, seed-hydrated Demo Mode**:

1. Open the [Chronos Live Link](https://chronos-435347523554.asia-south1.run.app/).
2. On the login/onboarding screen, click **"Try Demo Mode"**.
3. Select one of the three personas: **Student**, **Professional**, or **Entrepreneur**.
4. The dashboard will instantly hydrate with a rich set of active data: Kanban boards, goals, habits, calendar events, and Time Warp forecasts.
5. **Activate Rescue Mode**: Click the glowing **Rescue Mode** toggle in the header. Watch the layout transition into the glassmorphic focus sandbox, view the computed schedule, and complete steps to watch your progress metrics update instantly!
6. **Open AI Sidebar**: Click the bottom-right AI bubble, talk to the agent, type commands, or request task decompositions to witness the real-time agentic loop in action.
